...
async def log_outgoing_request(request: httpx.Request) -> None:
    ...
    logger.info(f"-> OpenAI {request.method.upper()} {request.url}")
...
async def log_incoming_response(response: httpx.Response) -> None:
    ...
    if not response.is_error:
        logger.info(f"<- OpenAI {response.status_code} {request.url} ({duration}s)")
    else:
        logger.error(f"X OpenAI {response.status_code} {request.url} ({duration}s)")
...
async def ratlimit_requests(response: httpx.Response) -> None:
    ...
    if remaining < 50:
        logger.warning(f"OpenAI rate limit getting low: {remaining} remaining")
